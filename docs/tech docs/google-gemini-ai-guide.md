    ...
    try {
      ...
      if (functionCalls && functionCalls.length > 0) {
        ...
        const followUpResult = await this.model.generateContent([
          { text: userMessage },
          { functionResponse: { name: functionCalls[0].name, response: functionResponses[0].response } }
        ])
...
export async function generatePerformanceReport(
  ...
  const totalMetrics = metrics.reduce((acc, curr) => ({
    ...
    clicks: 0, conversions: 0, reach: 0, impressions: 0, cost: 0
  })
    ...
    <div className="space-y-4">
      ...
      {content && (
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
          ...
          {isGenerating && (
            <div className="mt-2 text-sm text-gray-500">생성 중...</div>
          )}
        </div>
      )}
    </div>
...
export function checkRateLimit(userId: string, limit: number = 10, window: number = 60000): boolean {
  const now = Date.now()
