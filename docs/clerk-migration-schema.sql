...
-- 12. 마이그레이션 상태 추적 테이블 생성
CREATE TABLE IF NOT EXISTS clerk_migration_status (
...
-- 13. 마이그레이션 상태 추적 인덱스
CREATE INDEX IF NOT EXISTS idx_clerk_migration_status_user_id ON clerk_migration_status(user_id);
CREATE INDEX IF NOT EXISTS idx_clerk_migration_status_clerk_user_id ON clerk_migration_status(clerk_user_id);
CREATE INDEX IF NOT EXISTS idx_clerk_migration_status_status ON clerk_migration_status(migration_status);

-- 14. 마이그레이션 로그 함수
CREATE OR REPLACE FUNCTION log_clerk_migration(
  p_user_id UUID,
  p_clerk_user_id TEXT,
  p_status TEXT,
...
DECLARE
  log_id UUID;
BEGIN
  INSERT INTO clerk_migration_status (
    user_id,
    clerk_user_id,
    migration_status,
...
-- 17. 슈퍼 관리자 설정 (alex@culinaryseoul.com)
-- 이 부분은 실제 Clerk 사용자 ID로 업데이트해야 합니다.
/*
INSERT INTO profiles (
  clerk_user_id,
  email,
...
VALUES (
  'clerk_user_id_for_alex', -- 실제 Clerk 사용자 ID로 변경
...
VALUES (
  'clerk_user_id_for_alex', -- 실제 Clerk 사용자 ID로 변경
  'SUPER_ADMIN',
  'clerk_user_id_for_alex',
  NOW()
)
ON CONFLICT (clerk_user_id, permission_type) DO NOTHING;
*/

-- 18. 마이그레이션 완료 후 정리 (선택사항)
-- 마이그레이션이 완전히 완료된 후 실행할 스크립트
/*
-- 기존 Supabase Auth 관련 컬럼 제거 (신중하게 진행)
-- ALTER TABLE profiles DROP COLUMN IF EXISTS user_id;
-- ALTER TABLE user_permissions DROP COLUMN IF EXISTS user_id;

-- 기존 RLS 정책 제거
-- DROP POLICY IF EXISTS "기존_정책_이름" ON profiles;
-- DROP POLICY IF EXISTS "기존_정책_이름" ON user_permissions;
*/

-- 마이그레이션 스크립트 완료
-- 이 스크립트를 실행한 후 Clerk 설정을 완료하고 애플리케이션을 테스트하세요.