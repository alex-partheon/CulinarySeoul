...
**매출관리(구현중)**: 토스페이먼츠 API를 통한 자동 매출 수집 및 관리 시스템은 현재 구현 중단 상태이며, 추후 개발 예정입니다.
...
export const successUrl = process.env.TOSSPAYMENTS_SUCCESS_URL || 'http://localhost:3000/payments/success'
...
class PaymentMonitor {
  ...
  private recordMetric(metric: PaymentMetrics) {
    ...
    // 실시간 알림 (성능 저하 감지)
    if (metric.duration > 10000) { // 10초 이상
